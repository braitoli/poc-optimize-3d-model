...
| `--reduce-normal-budget` | | `20` | How far Step 3 may let the surface turn away from the original, in degrees at the 99th percentile. A collapse can round a crease away while barely moving the surface, so this is what keeps sharp edges and narrow slots. 20° is the widest setting at which the rate of faces lit unlike their neighbours stays at the floor on every model measured (per 10,000 faces: dinoki 0.7, vosiruto 1.2, zelvaron 3.4; at 30° the hard-surface ones jump to 5.4 and 8.5). Past roughly 30° the 0.1% surface budget binds instead and the setting stops doing anything |
